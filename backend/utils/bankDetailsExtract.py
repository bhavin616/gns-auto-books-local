...
def try_parse_date(value: str):
    ...
    for fmt in ["%b %d %Y", "%b %d, %Y", "%b %d %y", "%m/%d", "%Y/%m/%d"]:
        try:
            # If the format is MM/DD, assume the current year
            if fmt == "%m/%d":
                # Assuming the current year if no year is provided
                current_year = datetime.now().year
                value = f"{current_year}/{value}"  # prepend current year
